...
@section('title', __('Create Person'))

@section('content')
    <x-forms.post :action="route('admin.person.store')">
        <x-backend.card>
            <x-slot name="header">
                @lang('Create Person')
            </x-slot>
            ...
            <x-slot name="body">
                <div class="form-group row">
                    ...
                    <div class="col-md-10">
                        <input type="text" name="name with type string" class="form-control" placeholder="{{  __('name with type string') }} " value="{{  old('name with type string') }} "  />
                    </div>
            ...
            <x-slot name="footer">
                <button class="btn btn-sm btn-primary float-right" type="submit">@lang('Create Person')</button>
            </x-slot>
        </x-backend.card>
    </x-forms.post>
@endsection